...
#if LWIP_ICMP
static struct nat_queue icmp_queue[NAT_ICMP_QUEUE_NUM] = {};
#endif
#if LWIP_TCP
static struct nat_queue tcp_queue[NAT_TCP_QUEUE_NUM] = {};
#endif
#if LWIP_UDP
static struct nat_queue udp_queue[NAT_UDP_QUEUE_NUM] = {};
#endif
...
nat_state_t nat_output(struct pbuf *p) {
  ...
  if (!conf_list) goto nat_state_direct;
...
static void nat_apply_in(struct pbuf *p, struct ip_hdr *ip_hdr, struct nat_queue *nat_queue) {
  ...
  if (netif_in->output(netif_in, q, (const ip4_addr_t *)&ip_hdr->dest) != ERR_OK) LWIP_DEBUGF(NAT_DEBUG | LWIP_DBG_TRACE | LWIP_DBG_LEVEL_SERIOUS, ("nat_apply_in(): failed to send modified packet\n"));
...
static void nat_apply_out(struct pbuf *p, struct ip_hdr *ip_hdr, struct nat_queue *nat_queue) {
  ...
  if (netif_out->output(netif_out, p, (const ip4_addr_t *)&ip_hdr->dest) != ERR_OK) LWIP_DEBUGF(NAT_DEBUG | LWIP_DBG_TRACE | LWIP_DBG_LEVEL_SERIOUS, ("nat_apply_out(): failed to send modified packet\n"));
  else LWIP_DEBUGF(NAT_DEBUG | LWIP_DBG_STATE, ("NAT applied\n"));
}
#endif
